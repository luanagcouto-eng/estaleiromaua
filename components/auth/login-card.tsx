"use client";

import { createClient } from "@/lib/supabase/client";
import { useState } from "react";
import Image from "next/image";

export default function LoginCard() {
  const [loading, setLoading] = useState(false);
  const supabase = createClient();

  async function handleGoogleLogin() {
    setLoading(true);
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback`,
        queryParams: { hd: "estaleiromaua.ind.br" },
      },
    });
    setLoading(false);
  }

  return (
    <div
      className="min-h-screen flex items-center justify-center p-4"
      style={{ backgroundColor: "#364B59" }}
    >
      <div
        className="bg-white rounded-3xl w-full max-w-[360px] p-8 flex flex-col items-center"
        style={{
          boxShadow:
            "0 20px 50px -12px rgba(0,66,126,0.18), 0 10px 15px -5px rgba(0,0,0,0.08)",
          animation: "fadeInScale 0.7s cubic-bezier(0.22,1,0.36,1) forwards",
        }}
      >
        {/* Logo */}
        <div className="mb-5">
          <Image
            src="/logo-maua.png"
            alt="Estaleiro Mauá"
            width={160}
            height={70}
            priority
            className="object-contain"
          />
        </div>

        {/* Título */}
        <div className="text-center mb-7 space-y-1">
          <h1
            className="text-[23px] font-black uppercase leading-tight text-slate-900"
            style={{ letterSpacing: "-0.01em" }}
          >
            Metas Estratégicas<br />2026
          </h1>
          <p
            className="text-[11px] font-bold uppercase text-slate-400"
            style={{ letterSpacing: "0.25em" }}
          >
            Portal do Colaborador
          </p>
        </div>

        {/* Botão principal — ACESSAR PAINEL */}
        <button
          onClick={handleGoogleLogin}
          disabled={loading}
          className="w-full bg-[#C8D5DC] border-2 border-[#C8D5DC] text-[#364B59] font-black py-4 rounded-[1.5rem] shadow-md hover:bg-[#364B59] hover:border-[#364B59] hover:text-white transition-all flex justify-center items-center group active:scale-95 disabled:opacity-70 mb-4"
        >
          {loading ? (
            <div className="h-5 w-5 border-2 border-[#364B59]/30 border-t-[#364B59] rounded-full animate-spin" />
          ) : (
            <div className="flex items-center gap-3">
              <span className="text-xs uppercase" style={{ letterSpacing: "0.3em" }}>
                Acessar Painel
              </span>
              <svg
                className="h-4 w-4 group-hover:translate-x-1 transition-transform"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M14 5l7 7m0 0l-7 7m7-7H3" />
              </svg>
            </div>
          )}
        </button>

        {/* Divisor OU UTILIZE */}
        <div className="relative w-full my-2">
          <div className="absolute inset-0 flex items-center">
            <span className="w-full border-t border-slate-100" />
          </div>
          <div className="relative flex justify-center">
            <span
              className="bg-white px-4 text-slate-400 font-bold uppercase"
              style={{ fontSize: "9px", letterSpacing: "0.15em" }}
            >
              OU UTILIZE
            </span>
          </div>
        </div>

        {/* Botão Google */}
        <button
          onClick={handleGoogleLogin}
          disabled={loading}
          className="w-full mt-2 bg-white border-2 border-slate-100 text-slate-700 font-bold py-4 rounded-[1.5rem] shadow-sm hover:border-[#364B59] hover:bg-slate-50 transition-all flex justify-center items-center gap-3 active:scale-95 disabled:opacity-70"
        >
          {loading ? (
            <div className="h-5 w-5 border-2 border-slate-200 border-t-slate-500 rounded-full animate-spin" />
          ) : (
            <>
              <svg className="h-5 w-5 shrink-0" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              <span className="text-[11px] uppercase" style={{ letterSpacing: "0.2em" }}>
                Entrar com Google
              </span>
            </>
          )}
        </button>

        {/* Rodapé */}
        <div
          className="mt-8 w-full text-center border-t border-slate-100 pt-6 text-slate-400 font-bold uppercase opacity-60"
          style={{ fontSize: "9px", letterSpacing: "0.4em" }}
        >
          ESTALEIRO MAUÁ
        </div>
      </div>

      <style>{`
        @keyframes fadeInScale {
          from { opacity: 0; transform: scale(0.98) translateY(10px); }
          to   { opacity: 1; transform: scale(1)    translateY(0);    }
        }
      `}</style>
    </div>
  );
}
